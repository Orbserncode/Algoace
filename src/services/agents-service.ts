...
export const StrategyCodingAgentConfigSchema = BaseAgentConfigSchema.extend({
  llmModelProviderId: z.string().optional().describe("ID of the configured LLM provider to use."), // Changed from llmModel string
...
export const ExecutionAgentConfigSchema = BaseAgentConfigSchema.extend({
  ...
  executionLogicPrompt: z.string().optional().describe("Optional prompt for AI-assisted execution adjustments (e.g., dynamic order sizing based on volatility)."),
});
...
export const DataAgentConfigSchema = BaseAgentConfigSchema.extend({
  dataProviderConfigId: z.string().optional().describe("ID of a specific data provider configuration (e.g., a specific broker or direct API)."), // More specific than just enum
  ...
  watchedAssets: z.array(z.object({
      brokerId: z.string().describe("ID of the broker providing this asset."), // Link asset to its broker
...
Recent Trades: {trades}`
  ).describe("Prompt for the analysis LLM. Use placeholders like {metrics}, {trades}."),
...
const mockAgents: Agent[] = [
  {
    ...
    config: StrategyCodingAgentConfigSchema.parse({ llmModelProviderId: 'llm-google-default', llmModelName: 'gemini-2.0-flash', enabledTools: ['Backtester'] }),
  },
  {
    ...
    config: ExecutionAgentConfigSchema.parse({ brokerConfigId: 'broker-alpaca-paper', enabledTools: ['OrderExecutor', 'PortfolioManager']}),
  },
  {
  ...
  },
  {
    ...
    config: ExecutionAgentConfigSchema.parse({ brokerConfigId: 'broker-alpaca-paper', maxConcurrentTrades: 3, orderRetryAttempts: 5, enabledTools: ['OrderExecutor', 'PortfolioManager'] }),
  },
  {
    ...
    config: AnalysisAgentConfigSchema.parse({ llmModelProviderId: 'llm-google-default', llmModelName: 'gemini-2.0-flash', enabledTools: ['PortfolioManager', 'TechnicalIndicatorCalculator']}),
  },
];

// Update mockAgentConfigs to use the new schemas
let mockAgentConfigs: Record<string, AgentConfig> = {
    'agent-gen-default': StrategyCodingAgentConfigSchema.parse({ llmModelProviderId: 'llm-google-default', llmModelName: 'gemini-2.0-flash', enabledTools: ['Backtester', 'TechnicalIndicatorCalculator'] }),
    'agent-exec-momentum': ExecutionAgentConfigSchema.parse({ brokerConfigId: 'broker-alpaca-paper', enabledTools: ['OrderExecutor', 'PortfolioManager'] }),
    'agent-data-main': DataAgentConfigSchema.parse({ dataProviderConfigId: 'broker-alpaca-paper', watchedAssets: [{brokerId: 'broker-alpaca-paper', symbol: 'AAPL'}, {brokerId: 'broker-alpaca-paper', symbol: 'MSFT'}, {brokerId: 'broker-alpaca-paper', symbol: 'GOOGL'}], useSerpApiForNews: true, fetchFrequencyMinutes: 5, enabledTools: ['MarketDataFetcher', 'WebSearcher'] }),
    'agent-exec-ai': ExecutionAgentConfigSchema.parse({ brokerConfigId: 'broker-alpaca-paper', maxConcurrentTrades: 3, orderRetryAttempts: 5, enabledTools: ['OrderExecutor', 'PortfolioManager'] }),
    'agent-risk-default': AnalysisAgentConfigSchema.parse({llmModelProviderId: 'llm-google-default', llmModelName: 'gemini-2.0-flash', enabledTools: ['PortfolioManager', 'TechnicalIndicatorCalculator', 'MarketDataFetcher']}),
};
...
export async function updateAgentConfig(agentId: string, config: AgentConfig): Promise<AgentConfig | null> {
    ...
    let isValid = false;
    switch (agent.type) {
        case 'Strategy Coding Agent': isValid = StrategyCodingAgentConfigSchema.safeParse(config).success; break;
        case 'Execution Agent': isValid = ExecutionAgentConfigSchema.safeParse(config).success; break;
        case 'Data Agent': isValid = DataAgentConfigSchema.safeParse(config).success; break;
        case 'Analysis Agent': isValid = AnalysisAgentConfigSchema.safeParse(config).success; break;
    }

    if (!isValid) {
        console.error(`Config for agent ${agentId} (type: ${agent.type}) is invalid:`, config, StrategyCodingAgentConfigSchema.safeParse(config).error);
        // In a real app, throw a more specific error or return detailed validation errors
        throw new Error("Configuration data is invalid for the agent type.");
    }

    console.log(`SERVICE: Updating config for agent ${agentId}`, config);
    mockAgentConfigs[agentId] = config;
    return config;
}
...
export async function addAgent(agentData: Omit<Agent, 'id' | 'tasksCompleted' | 'errors'>): Promise<Agent> {
    ...
    let defaultConfig: AgentConfig;
     switch (newAgent.type) {
        ...
        case 'Execution Agent': defaultConfig = ExecutionAgentConfigSchema.parse({ brokerConfigId: "" }); break; // Ensure required fields have valid defaults or are made optional
